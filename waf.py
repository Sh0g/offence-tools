...
def calc_logging_level(verbosity):
    default = 40  # Default logging level for errors
    level = default - (verbosity * 10)
    if level < 0:
        level = 0
    return level
...
def main():
    print("Enter the URL:")
    url = input()  # Get URL from standard input
    print("Enter verbosity level (0-3):")
    verbosity = int(input())  # Get verbosity level from standard input
    start_time = time.time()


    # Set up logging level based on verbosity
    logging_level = calc_logging_level(verbosity)
    logging.basicConfig(level=logging_level)
    log = logging.getLogger('wafw00f')
    url = f"http://{url}"

    # Assuming other necessary parameters are set by default or through similar input prompts
    attacker = WAFW00F(url, debuglevel=verbosity, path='/', followredirect=True, extraheaders={}, proxies=None)

    # Perform the WAF check
    waf, xurl = attacker.identwaf(findall=False)
    output=[]
    if waf:
        print(f'[+] The site {url} is behind {waf} WAF.')
        output.append (f'[+] The site {url} is behind {waf} WAF.')
    else:
    ...
    make_report("Web Firewall Application Detector", url, output, "waf_report.pdf", date)
